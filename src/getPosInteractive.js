...
//let tsl = language.tsl;

function getPosInteractive(promptText,type) {
    let langType = configuration.readGlobalConfig("languageType", -1);
    let gotPos = false;
    //pos[0] 宽, pos[1] 高
    let pos = [];
    ...
    let confirmWindow = floaty.rawWindow(
        <frame gravity="left|top">
            <vertical bg="#88ffffff">
                ...
                <button id="confirmBtn" style="Widget.AppCompat.Button.Colored" text="" />
                <button id="cancelBtn" style="Widget.AppCompat.Button.Colored" text="" />
            </vertical>
        </frame>
    );
    ...
    ui.run(() => {
        confirmWindow.promptText.setText(tsl(25, langType,"Please Tap ") + promptText);
        confirmWindow.confirmBtn.setText(tsl(77,langType,"OK"));
        confirmWindow.cancelBtn.setText(tsl(78,langType,"Cancel"));
    ...
    while (!confirmed) {
        ...
        ui.run(function () {
            if (!gotPos) {
                ...
                confirmWindow.promptText.setText(tsl(31, langType,"Current coordinates:") + pos.toString());
            } else {
                confirmWindow.promptText.setText(tsl(31, langType,"Current coordinates:") + pos.toString() + tsl(32, langType,", Click 'OK' to finish, click 'Cancel' to retrieve again.")); 
            }
        });
    }
